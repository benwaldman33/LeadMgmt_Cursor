...
async function checkOperationMappings() {
  ...
  try {
    ...
    const operations = await prisma.operationServiceMapping.groupBy({
      by: ['operation'],
      _count: {
        operation: true
      }
    ...
    if (operations.length === 0) {
      console.log('No operations found in the database.\n');
    } else {
      operations.forEach(op => {
        console.log(`${op.operation}: ${op._count.operation} mappings`);
      });
      console.log('');
    }
